...
int check(int bot, int user) {
  int state = (15 + bot - user) % 15; //checks for the remainder
  if (state == 0) {
    return 0; //bot = user
  } else if (state <= 7) {
    return -1; //difference in selection is not more than 7, winning?
  } else if (state > 7) {
    return 1; //difference in selection more than 7 losing??
  }
}

void win() {
  system("cat flag.txt"); //we want to call this
}
...
restart: ;
    ...
    char inp[8]; //declared buffer
    int score = 0; //declar score
    int user;
    int bot;
    srand(time(NULL));
    ...
    bot = rand() % 15; //takes remainder of 15
    printf("\nI choose %s\n", choices[bot].choice);
    ...
    for (int i = 0; i < 5; i++) {
      bot = rand() % 15;
      printf("I choose %s\n", choices[bot].choice);
      gets(inp); //inp declared for input
      user = atoi(inp) - 1; //converts string to integer
      printf("You chose %s\n", choices[user].choice);

      if ((user < 0) || (user > 14)) {
        puts("What are you doing? I don't want to play with you anymore.");
        exit(0); //can edit GOT. not useful
      }
    ...
    if (score < 5) {
      ...
      alarm(0);
      goto restart; //natural restart of program
    } else {
      puts("ggwp");
    }
    return 0;
